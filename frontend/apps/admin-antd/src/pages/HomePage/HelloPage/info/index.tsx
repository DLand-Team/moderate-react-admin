...
export const Info = ({
  ...
  const checkElementVisibility = () => {
    const flag1 =
      ref1.current &&
      isInViewPortOfTwo(ref1.current, containerRef.current?.clientHeight!);
    const flag2 =
      ref2.current &&
      isInViewPortOfTwo(ref2.current, containerRef.current?.clientHeight!);
    const flag3 =
      ref3.current &&
      isInViewPortOfTwo(ref3.current, containerRef.current?.clientHeight!);
    const flag4 =
      ref4.current &&
      isInViewPortOfTwo(ref4.current, containerRef.current?.clientHeight!);
    if (flag2) {
      setStep(2);
    } else if (flag3) {
      setStep(3);
  ...
  useEffect(() => {
    containerRef.current!?.addEventListener("scroll", checkElementVisibility);
  }, []);
  return (
    <div
      style={{
