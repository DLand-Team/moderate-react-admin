...
const NavTabs = () => {
  ...
  useLocationListen(
    (location) => {
      const tabItemsTemp = cloneDeep(getStore("appStore").tabItems).filter(
        (item) => {
          return item;
        },
      );
      const { pathname } = location;
      const routeConfigItem = routerHelper.getRoutItemConfigByPath(pathname);
      ...
      const parentConfigItem = routerHelper.getRoutItemConfigById(parentId!);
      if (!isTab) {
        ...
        if (indexRoute) {
          ...
          tabItemsTemp[targetIndex] = {
            ...tabItemsTemp[targetIndex],
            location,
            label: routerHelper.getRouteTitleByKey(id as ROUTE_ID_KEY) || "",
          };
        ...
        if (
          ...
          if (parentConfigItem?.id) {
            ...
            insertIndex =
              insertIndexTemp !== -1 ? insertIndexTemp + 1 : insertIndex;
          }
  ...
  const onDragEnd = ({ active, over }: DragEndEvent) => {
    ...
    const activeIndex = tabsHistoryTemp.findIndex((i) => i.key === active.id);
            ...
            <DefaultTabBar {...tabBarProps}>
              {(node) => {
                return (
                  <DraggableTabNode
                    {...node.props}
                    zIndex={tabBarProps.activeKey == node.key ? 1 : 0}
                  ...
                  >
                    <Dropdown
                      onOpenChange={(open) => {
                        if (open) {
                          currentTabRef.current = node.key!;
                        } else {
                      ...
                      menu={{
                        ...
                        onClick() {
                          ...
                          setRefreshKey([key.split("/").slice(-1)[0]]);
                        },
                      }}
                      trigger={["contextMenu"]}
                    >
