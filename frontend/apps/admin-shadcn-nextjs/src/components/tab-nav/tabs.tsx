...
export const CustomTabs: React.FC = () => {
  ...
  const onDragEnd = ({ active, over }: DragEndEvent) => {
    if (active.id !== over?.id) {
      const activeIndex = historyRoutes.findIndex((i) => i.id === active.id);
        ...
        <div className="flex items-center space-x-2 bg-muted/60 rounded-t-lg px-2 pt-1 border-b-2 border-b-blue-100">
          {Object.values(historyRoutes).map((item) => (
            <DraggableTabNode key={item.id} data-node-key={item.id}>
              ...
              >
                <div className={cn("truncate", isAlowDelete ? "pr-6" : "")}>
                  {t(item.meta?.title!) || ""}
                </div>
                {isAlowDelete && (
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      emit("appStore").deleteHistoryRouteAct({
                        url: item.url!,
                      });
                    }}
                    type="button"
                  >
                    ...
                    />
                    <div className="sr-only">关闭 {item.meta?.title || ""}</div>
                  </button>
                )}
              </div>
            </DraggableTabNode>
