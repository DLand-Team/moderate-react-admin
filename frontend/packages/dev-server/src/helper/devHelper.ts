...
class devHelper {
  ...
  toRegistePage = (newItem, path, pageName) => {
    ...
    // 写入组件表
    {
      ...
      fs.writeFileSync(pagePath, this.toFromat(newCode).replace(/\'/g, '"'));
    }
  };
  // 注册插件页面信息
  toRegisterPluginPage = ({
    ...
    // 第一步：注册name
    {
      const pageCode = this.toFromat(fs.readFileSync(namePath).toString());
      let newCode = pageCode;
    ...
    // 第二步骤：注册路由
    {
      ...
      // 获得路由数据
      let routeConfig = this.getRouteConfig(pathHelper.pluginRouterConfigPath);
      routeConfig[pageName] = newItem;
      // 做个记号，然后替换
      let configStr = pageCode.split("=");
  ...
  toRemovePluginPage = ({
    ...
    // 干掉路由配置数据
    {
      ...
      let oldCode = this.toFromat(fs.readFileSync(routeConfigPath).toString());
    ...
    // 干掉page
    {
      ...
      let oldCode = this.toFromat(fs.readFileSync(pageIndexPath).toString());
      let newCode = oldCode.replace(reg, "").replace(reg2, "");
  ...
  toRegisterTheme = (pluginName: string, themeName) => {
    ...
    newCode = newCode.replace(registerSign, themeName + "\n" + registerSign);
    fs.writeFileSync(
  ...
  toRegisterI18n = (pluginName: string, name: string, lang: string) => {
    ...
    fs.writeFileSync(langRootPath, this.toFromat(newCode).replace(/\'/g, '"'));
  };
  ...
  toBuildPage = ({
    ...
    let filePath = path.resolve(pathHelper.webPagesPath, pagePath, pageName);
  ...
  convertToRouteStructConfig = (data) => {
    return JSON.stringify(this.convertToRouteStructConfigLoop(data)).replace(
      /\"/g,
      "",
    );
  };
  ...
  toreWritePluginRouteStructData = () => {
    ...
    this.readFileDirForPagesLoop(pathHelper.adminPlugins, pluginPagesPathArr);
    let result = {};
  ...
  readI18n(pluginName: string, lang: string): string[] {
    ...
    if (files.includes("i18n")) {
      const list = fs
        ...
        .filter((item) => {
          const stats = fs.statSync(pathStr + "/i18n/" + lang + "/" + item);
          if (stats.isDirectory()) {
            return true;
          }
        })
