...
export class Music2Effect implements MusicEffect {
	...
	public update(voiceHigh: Uint8Array): void {
		...
		for (let i = 0; i < this.all; i++) {
			...
			this.ctx.moveTo(
				Math.sin((i * this.du) / 180 * Math.PI) * rv1 + this.canvas.width / 2,
				-Math.cos((i * this.du) / 180 * Math.PI) * rv1 + this.canvas.height / 2
			);
			this.ctx.lineTo(
				Math.sin((i * this.du) / 180 * Math.PI) * rv2 + this.canvas.width / 2,
				-Math.cos((i * this.du) / 180 * Math.PI) * rv2 + this.canvas.height / 2
			);
		}
		this.ctx.stroke();
	}
