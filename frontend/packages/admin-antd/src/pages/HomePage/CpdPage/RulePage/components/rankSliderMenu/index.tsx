...
// targetRankId
const RankSliderMenu = (props: any) => {
    const { handleItDelete, handleOpenChange, branchName } = props;
    const { isDetail: isJustShow, isEditing } = useFlat(
    ...
    const actionName = useRef(ActionType.move);
            ...
            content: (
                ...
                >
                    {itByRankList[dragItem.rankId - 1].length !== 1 && (
                        <Radio value={ActionType.move}>
                            {t("rule:sortItem_move")}
                        </Radio>
                    )}
                    <Radio value={ActionType.copy}>
                        {t("rule:sortItem_copy")}
                    </Radio>
                </Radio.Group>
            ),
            onOk: () => {
    ...
    const handleRankSwitch = (dragItem: DragItem, trgetItem: DragItem) => {
        Modal.confirm({
            ...
            onOk: () => {
                let newItem = cloneDeep(dragItem.itemData);
                // move
                if (actionName.current == ActionType.move) {
        ...
        <div className={styles.container}>
            ...
            <DndProvider backend={HTML5Backend}>
                <Menu
                    ...
                    onClick={({ key, keyPath }) => {
                        if (isEditing) {
                            message.warning(t("common:blog.editing"));
                        } else {
                            setTargetRankId(
                                Number(keyPath[keyPath.length - 1])
                            );
